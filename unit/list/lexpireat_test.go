...
var _ = Describe("lexpireat Cmd", func() {
	...
	It("ERR invalid expire value", func() {
		Expect(c.Do(ctx, "LEXPIREAT", "k110", "100").Err().Error()).To(ContainSubstring("ERR invalid expire value"))
	})

	It("ERR value is not an integer or out of range", func() {
		Expect(c.Do(ctx, "LEXPIREAT", "k110", "1a0").Err().Error()).To(ContainSubstring("ERR value is not an integer or out of range"))
	})
})
